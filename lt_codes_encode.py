...
#        print(type(curr_symbol.data))
        file.close()

#        print(file_symbols[0].data)
#        print(type(file_symbols[0].data))

        root, ext = os.path.splitext(args.filename)

    with open(root + "ENCRYPTED" + ext, "w") as f:
        for symbol in file_symbols:
            f.write(str(symbol.index) + ", " + str(symbol.degree) + ", " + str(symbol.data))
            f.write("\n")
        f.write(str(file_blocks_n) + "\n")
        f.write(str(filesize))
        print("Encrypted file created")
        f.close()

    
    #np.savetxt("numpyArray.txt", symbol.data)

    #print(symbol.data)

    #test = symbol.data

    #test.tofile("numpyArray.bin", ", ")

    #with open("numpyArray.bin", "r") as f:
    #    print(f.read())
